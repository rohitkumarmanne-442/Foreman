...
/**
 * foreman wrap --http — attestation for REMOTE MCP servers.
 *
 * Remote (streamable-HTTP / SSE) servers can't be wrapped as a stdio child,
 * so this runs a local relay instead: point your agent at the printed
 * 127.0.0.1 URL, and every JSON-RPC exchange is forwarded to the real
 * server while receipts are signed and chained exactly like stdio wraps —
 * including tools/list fingerprinting for rug-pull detection.
 *
 * JSON responses are attested per-call. SSE responses stream through
 * unbuffered; the terminal event carrying the request's id is attested
 * once the stream completes. GET/DELETE (session streams) pass through.
 */
export function runHttpProxy(name: string, target: string, listenPort = 0): Promise<{ port: number; server: http.Server }> {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      res.end(JSON.stringify({ error: `foreman http wrap: upstream unreachable — ${e instanceof Error ? e.message : e}` }));
      return;
    }
    ...
    const attest = (obj: { result?: unknown; error?: unknown } | null) => {
      if (parsed?.method) recordExchange(name, runId, { method: parsed.method, params: parsed.params, t0 }, obj ?? {}, appendEvent);
    };
